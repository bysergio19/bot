	...
	async execute(client, message, args, { config, Ticket }) {
		...
		if (config.commands.close.confirmation) {
			...
			let pre = fs.existsSync(paths.text) || fs.existsSync(paths.log)
				? `You can view an archived version later with \`${config.prefix}transcript ${ticket.id}\``
				: '';
			...
			collector.on('collect', async () => {
				...
				confirm.edit(
					new MessageEmbed()
						...
						.setDescription('The channel will be deleted in some seconds.')
						.setFooter(guild.name, guild.iconURL())
				);
				...
				if (!success) {
					...
					confirm.edit(
						new MessageEmbed()
							...
							.setAuthor(message.author.username, message.author.displayAvatarURL())
							.setTitle('❌ **Expired**')
							.setDescription('You took longer than expected to react!')
							.setFooter(guild.name, guild.iconURL()));
			...
			if (config.transcripts.text.enabled || config.transcripts.web.enabled) {
				...
				if (u) {
					...
					try {
						...
						log.warn(`I couldn't send the transcript to ${u.tag} DM's.`);
					}
