...
module.exports = {
	name: 'transcript',
	description: 'Descarga la conversacion de un ticket',
	usage: '<ticket-id>',
		...
		if (!ticket) {
			return message.channel.send(
				new MessageEmbed()
					...
					.setDescription('I couldn\'t find that ticket ID!')
		...
		if (message.author.id !== ticket.creator && !message.member.roles.cache.has(config.staff_role)) {
			return message.channel.send(
				new MessageEmbed()
					...
					.setDescription(`You don't have permission to view ticket ${id}.`)
					.setFooter(guild.name, guild.iconURL())
			);
		}
